...
def generate_image(
    ...
    mu  = torch.tensor([-0.1280, -0.0889, -0.1179, -0.1231,  0.1712,  0.5867, -0.4217,  0.3073,
            -0.1769,  0.5607,  0.2017,  0.2731, -0.1683, -0.4220, -0.6509, -0.1403],
            device='cuda:0')
    std = torch.tensor([0.8401, 1.5132, 0.9567, 0.5362, 0.5751, 0.6432, 1.3855, 1.0647, 0.7470,
            1.0976, 0.7545, 0.7611, 1.1662, 1.1019, 0.9952, 1.0370],
            device='cuda:0')
    ...
    print("Feature statistics mean/std:", mu,std)
    ae = ae.to(device)
    with torch.no_grad():
        if is_fp8(ae_dtype):
            with accelerator.autocast():
...
if __name__ == "__main__":
   ...
   # "A candid photo of a person laughing, with a genuine expression, in a cozy coffee shop, with warm, inviting lighting and a soft focus on the background."
    ...
    # with torch.no_grad():
    #     for name, param in model.named_parameters():
    #         if "out" in name or "final" in name:   # z.B. Decoder-Output-Layer
    #             scale = param.data.abs().mean()
    #             param.data /= (scale + 1e-6)
