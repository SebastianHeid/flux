...
class DoubleStreamBlockPruned(nn.Module):
    ...
    def _forward(
        ...
        if self.flag_img_proj:
            img_projection = (img_attn @ self.img_attn.proj_linear_A @ self.img_attn.proj_linear_B) + self.img_attn.proj_bias
        else: 
            img_projection = self.img_attn.proj(img_attn)
            
        img = img + img_mod1.gate * img_projection
        ...
        if self.flag_txt_proj:
            txt_projection = (txt_attn @ self.txt_attn.proj_linear_A @ self.txt_attn.proj_linear_B) + self.txt_attn.proj_bias
        else: 
            txt_projection = self.txt_attn.proj(txt_attn)
            
        txt = txt + txt_mod1.gate * txt_projection
