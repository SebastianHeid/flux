...
sys.path.append("/home/hd/hd_hd/hd_om233/SVD/flux")
...
def train(args):
    ...
    flux = myCode.modify_model.modify_model(flux, 
    ...
    #with torch.no_grad():
    null_inp = tokens_and_masks = flux_tokenize_strategy.tokenize("")
    null_inp_device =[ids.to(accelerator.device) for ids in null_inp]
    cached_null_text_encoder_conds = text_encoding_strategy.encode_tokens(
                        flux_tokenize_strategy, [clip_l, t5xxl], null_inp_device, args.apply_t5_attn_mask
                    )

    if args.full_fp16:
        cached_null_text_encoder_conds = [c.to(weight_dtype) for c in cached_null_text_encoder_conds]
    
    # Unpack for clarity, these are the cached tensors
    (
        cached_null_l_pooled,
        cached_null_t5_out,
        cached_null_txt_ids,
        cached_null_t5_attn_mask,
    ) = cached_null_text_encoder_conds
    accelerator.print("Null embeddings cached.")
    
    trainable_params = sum(p.numel() for p in flux.parameters() if p.requires_grad)
    total_params = sum(p.numel() for p in flux.parameters())
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*training_models):
                ...
                if args.KD_flag:
                    ...
                    if single_block_features:
                    
                        l_final_abs_nograd = l_final.detach().abs()
                        norm_single_feat_loss = normalized_feature_distillation_loss(ref_single_block_features, single_block_features, l_final_abs_nograd)
                        loss +=  args.single_loss_weighting * norm_single_feat_loss
                
                    
                    if double_block_features:
                    
                        l_final_abs_nograd = l_final.detach().abs()
                        norm_double_feat_loss = normalized_feature_distillation_loss(ref_double_block_features, double_block_features, l_final_abs_nograd)
                    ...
                    # --- START: Modified loss logic based on the paper ---
                    
                    # 1. Get unconditional prediction v_eta(empty)
                    
                    # Expand cached null embeddings to match batch size
                    
                    # We run this pass with no_grad() as we only need the output
                    # to correct the conditional prediction.
                    
                    with accelerator.autocast():
                        uncond_model_pred = flux(
                            img=packed_noisy_model_input,
                            img_ids=img_ids,
                            txt=cached_null_t5_out,
                            txt_ids=cached_null_txt_ids,
                            y=cached_null_l_pooled,
                            timesteps=timesteps / 1000,
                            guidance=guidance_vec,
                            txt_attention_mask=cached_null_t5_attn_mask,
                        )
                    
                    # Unpack and apply prediction type (same as conditional pred)
                    uncond_model_pred = flux_utils.unpack_latents(uncond_model_pred, packed_latent_height, packed_latent_width)
                    uncond_model_pred, _ = flux_train_utils.apply_model_prediction_type(
                        args, uncond_model_pred, noisy_model_input, sigmas
                    )

                    # 2. Apply the correction formula from the paper
                    # v_theta = 1/(1+w) * (v_eta(c) + w * v_eta(empty))
                    # model_pred is v_eta(c)
                    # uncond_model_pred is v_eta(empty)
                    
                    w = float(args.guidance_scale)
                    # Ensure dtypes match for the operation
                   
                    corrected_model_pred = (1.0 / (1.0 + w)) * (model_pred + w * uncond_model_pred.to(model_pred.dtype))

                    # 3. Calculate loss using the corrected prediction against the original target
                    ...
                    # Use corrected_model_pred instead of model_pred
                    loss = train_util.conditional_loss(corrected_model_pred.float(), target.float(), args.loss_type, "none", huber_c)
